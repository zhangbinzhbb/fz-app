  ...
  >
    ...
    >
      ...
      <div class="tool-con">
        ...
        <div v-if="item.commentCount>0 && false" class="tool-con-lv3">
          <ul class>
            <li v-for="(comment,idx) in item.progressCommentList" :key="idx">
              <span class="name">{{ comment.createName }}:</span>
              <div v-if="flag" class="oneline" v-html="comment.commentHtml"/>
              <div v-else v-html="comment.commentHtml"/>
              <img
                v-if="comment.isEditDelete=='0' && !flag"
                style="padding-left:0"
                src="@/assets/images/icon/trash.png"
                alt
                class="vant-del"
                @click.stop="iconDelet(comment.commentId,'评论删除')"
              >
            </li>
          </ul>
        </div>
    ...
    <bottom v-if="!pageInfo.isLoading&&isLoadAll&&pageInfo.list.length!=0&&pageInfo.list.length>0"/>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    const processCacheData = this.storage.get('process-cache-data')
    if (processCacheData) {
      this.avatorParams = processCacheData.avatorParams
      this.pageInfo = processCacheData.pageInfo
