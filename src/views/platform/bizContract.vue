<template>
  <div class="has-comment">
    ...
    <div class="comment-content no-comment">
      <div v-for="(item,index) in projContractFileAppList" :key="index" class="item">
        <h2 @click="turnToDetail(item)">
          <span class="h-left">
            <img src="@/assets/images/liucheng@2x.png" alt="">
            <span class="van-ellipsis"> {{ item.contractName }}</span>
          </span>
          <van-icon
            name="arrow"
        ...
        <div class="inner-item">
          ...
          <div class="label p">主要内容：</div>
          <div v-if="item.contractContent">
            <div v-if="item.contractContent.length>66 && !item.isCheck" class="p">
              {{ item.contractContent | contentFilter }}...<span class="expand-btn" @click.stop="expandChange(item,index)">展开</span>
            </div>
            <div v-else class="p">
              {{ item.contractContent }} <span v-if="item.contractContent.length>66" class="expand-btn" @click.stop="expandChange(item,index)">收起</span>
            </div>
          </div>
          <div v-else>
            --
      ...
      <blank v-if="projContractFileAppList.length===0&&!showLoading"/>
...
export default {
  ...
  data() {
    return {
      projContractFileAppList: [],
      catTitle: '合同签署',
      showLoading: true
    }
  },
  created() {
    this.getProjContractFileAppList()
  },
  methods: {
    turnToDetail(item) {
      ...
      const cmd = 'MyTaskEipgetContractInfo'
      const taskId = ''
      const instId = item.processInstId
      const BIZKEY = item.applyNo
      const FORMURI = ''
    ...
    expandChange(item, index) {
      ...
      this.$set(this.projContractFileAppList[index], 'isCheck', check)
    },
    // 合同签署
    getProjContractFileAppList() {
      this.showLoading = true
      const URL = '/founderOaApplyInfoList'
      this.$axios.get(
        URL,
        {
        ...
        res => {
          ...
          this.projContractFileAppList = res.data
          this.showLoading = false
        }
      )
    }
