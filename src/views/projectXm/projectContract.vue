<template>
  <div class="has-comment">
    ...
    <div class="comment-content no-comment">
      <div v-for="(item,index) in projContractFileAppList" :key="index" class="item" @click="turnToDetail(item)">
        <h2>
          <span class="h-left">
...
export default {
  ...
  created() {
    ...
    this.getProjContractFileAppList(projectObject, inveId)
  },
  methods: {
    ...
    // 合同签署
    getProjContractFileAppList(projectObject, inveId) {
      this.showLoading = true
      const URL = '/projContractFileAppList'
      this.$axios.get(
        URL,
        {
          pageSize: 1000,
