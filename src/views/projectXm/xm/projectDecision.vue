  ...
  <div class="has-comment">
    <div class="comment-content no-comment">
      ...
      <div v-if="article.part1" class="reg-info">
        ...
        <div class="item item-column">
          <span class="label">是否中小企业</span>
          <div class="ctx">{{ model_extend.isSmEnterprise? model_extend.isSmEnterprise:'--' }}</div>
        </div>
        <div v-if="model_extend.isSmEnterprise && model_extend.isSmEnterprise ==='是'" class="item item-column">
          <span class="label">中小企业则上传相关附件</span>
          <div class="ctx"><attachment :data="model_extend.fileListsSM"/></div>
        </div>
        <div class="item item-column">
          <span class="label">是否高新企业</span>
          <div class="ctx">{{ model_extend.isHnTech? model_extend.isHnTech:'--' }}</div>
        </div>
        <div v-if="model_extend.isHnTech && model_extend.isHnTech ==='是'" class="item item-column">
          <span class="label">高新企业发证时间/高新企业失效日期</span>
          <div class="ctx">{{ model_extend.apprDtate || '--' }}/{{ model_extend.effectiveDate || '--' }}</div>
        </div>
        <div class="item item-column">
          <span class="label">运营主体所在地址</span>
          <div class="ctx">{{ model.addressDetails? model.addressDetails:'--' }}</div>
        </div>
      </div>
      <!-- 项目信息 -->
      <div class="sub-title">
      ...
      <!-- 决策信息 -->
      <div class="sub-title">
        <img src="@/assets/images/icons/icon8@2x.png">
        决策信息
        <van-icon
      ...
      <div v-if="article.part3" class="reg-info">
        ...
        <div class="item item-column">
          <span class="label">尽调报告</span>
          <div class="ctx"><attachment :data="model2.makeFullReportList"/></div>
        </div>
        <div class="item item-column">
          <span class="label">附件</span>
          <div class="ctx"><attachment :data="model2.decisionFileLists"/></div>
        </div>
      </div>
      <!-- 决议结果 -->
      <div class="sub-title">
  ...
  watch: {
    modelData: {
      handler(newValue, oldValue) {
        this.model = this.modelData.baseInfoSnapshotDVO
        this.model1 = this.modelData.projInfoSnapshotDVO
        this.model2 = this.modelData.ProjAppInfoSnapshotDVO
        this.model3 = this.modelData.custMemberSnapshotDVO
        this.model4 = this.modelData.decisionInfoSnapshotDVO
        this.model_extend = this.modelData.baseInfoExtendDVO
      },
      deep: true
    }
  },
