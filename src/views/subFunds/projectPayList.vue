<template>
  <div class="project-quality-list">
    <div v-for="(item,index) in list" :key="index" class="item-box" @click="turnToDetail(item,groupId)">
      <div class="h-top">
        <span class="h-left"><img src="@/assets/images/liucheng@2x.png" alt="">{{ item.investor }}</span>
        <span class="h-right">
          {{ item.payDt }}
<!--          <img src="@/assets/images/cc@2x.png" alt="">-->
          <van-icon
            name="arrow"
            class="arrow-icon"
          />
...
export default {
  ...
  methods: {
    turnToDetail(item, groupId) {
      ...
      let cmd = ''
      if (groupId == 18) {
        cmd = 'PaymentApproveXX'
      } else if (groupId == 1) {
        cmd = 'PaymentReviewCK_Frist'
      }
      const taskId = ''
