...
window.appcan && appcan.define('window', function ($, exports, module) {
  ...
  */
  ...
  function openPopover(name, dataType, url, data, left, top, width, height, fontSize, type, bottomMargin, extraInfo) {
    ...
    var platformName = 'Android2.2+'
    // fixed ios bug
    if (platformName && platformName.toLowerCase().indexOf('ios') > -1) {
    ...
    //pym.js
    var pymParent = new pym.Parent(name, 'child.htm', {});
...
appcan.define('icache', function ($, exports, module) {
  ...
  iCache.prototype = {
    ...
    _success: function (fpath, session) {
      ...
      if (session.dom && session.dom.length) {
        session.dom = document.getElementById(session.dom)
        switch (session.dom.tagName.toLowerCase()) {
          case 'img':
            session.dom.attributes['src'].nodeValue = fpath
            break;
          default:
            // session.dom.css("background-image", "url(file://" + fpath + ") !important");
            session.dom.style.backgroundImage = 'url(' + fpath + ')';
            break
        }
      }
    ...
    _download: function (session) {
      ...
      var ftype = session.url.substring(session.url.lastIndexOf('.'), session.url.length)
      var fpath = self.realpath + '/icache/' + fid + ftype
    ...
    run: function (option) {
      ...
      for (var i = 0; i < urls.length; i++) {
        option.url = urls[i].url
        option.dom = urls[i].domId
        var session = $.extend({
          id: ('' + (opid++)),
          status: 0
        }, option, true)
